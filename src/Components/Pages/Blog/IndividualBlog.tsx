      ...
      <FeatureCard>
        <div className="space-y-5">
          <Link href={`${ASKME_URL}/blog/${blog?.slug}`} className="flex">
            {blog?.featuredImage?.fileUrl_medium ? (
              <Image className="w-full max-h-96" src={`https://${process.env.BUCKET_NAME_AWS}.s3.${process.env.REGION_AWS}.amazonaws.com/${blog?.featuredImage?.fileUrl_medium}`} alt={blog?.subject} width={600} height={200} />
            ) : (
