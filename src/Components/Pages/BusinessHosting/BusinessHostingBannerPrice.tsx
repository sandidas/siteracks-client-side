...
const BusinessHostingBannerPrice: FC<IProps> = ({ products, isLoading, isError }) => {
  ...
  const getService = services.find((p: IProduct) => p?.nameSlug?.includes("businessHosting"));
  // select single package from service
  const getPackage = getService?.packages?.find((p) => p?.typeSlug?.includes("startUpBusinessHosting"));
