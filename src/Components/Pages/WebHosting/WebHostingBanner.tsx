...
const WebHostingBanner: FC<IProps> = ({ product, isLoading, isError }) => {
  return (
    <div className="max-w-screen-2xl mx-auto px-5 grid grid-cols-1 xl:grid-cols-12 gap-5">
      {/* left column  */}
      <div className="xl:col-span-7 flex flex-col items-center xl:items-start group max-w-4xl gap-8 xl:gap-3 pb-[10vh] xl:py-[7vh] mx-auto min-h-screen justify-center">
        <h1 className="flex flex-col text-4xl lg:text-6xl md:leading-tight lg:leading-tight text-title font-bold text-center xl:text-left">
          <span>Blazing Fast & Secure</span>
          <span className="text-primary"> Cloud Web Hosting</span>
          {/*  */}
        </h1>

        <p className="text-text lg:text-base hidden xl:block  text-center xl:text-left">Enjoy the lightning-fast speed and unparalleled security for your website. The best web hosting packages for startups and small business.</p>
        ...
        <ul className="text-text gap-2 flex flex-wrap flex-col items-center xl:items-start text-center xl:text-left">
          ...
          <li className="bg-green-500/20 rounded-sm px-2 text-title text-sm xl:text-base">Install Wordpress in 1-Click</li>
